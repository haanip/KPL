<?php
  ...
  $id = $_GET['id'] ?? NULL; // Mendapatkan idpost yang dilewatkan ke url
  ...
  if (!isset($_POST["submit"])) {
    ...
    $query = "SELECT * FROM post where judul = '$id'";
    $result = $db->query($query);
    ...
    if ($valid) {
      ...
      // Asign a query
      $query = "UPDATE post SET judul='".$judul."', idkategori='".$kategori."', isi_post='".$isi_post."', file_gambar='".$nama."', tgl_update='".date("Y-m-d")."' WHERE idpost='$id'";
      $result = $db->query($query);
...
<!--                         <option value="1" <?php if (isset($idkategori) && $idkategori=="1") echo 'selected="true"';?>>Fiksi</option>
                        ...
                        <?php
                          ...
                          while($row2 = $result2->fetch_object()){
                            echo "<option value='".$row2->idkategori."'<?php if(isset($idkategori) && $idkategori=='".$row2->idkategori."') echo selected='true';?>".$row2->nama."</option>";
                          }
                        ?>
                    </select>
                </div>
